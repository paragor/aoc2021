...
func main() {
	...
	cache := generatesCache(pairs, src, CacheLen)
	//fmt.Println(cache)
	template := strings.TrimSpace(all[0])
	for i := 0; i < 10; i++ {
		template = concurrentCalculate(pairs, template, 12, cache)
...
func concurrentCalculate(pairs map[string]rune, template string, count int, cache map[string]string) string {
	...
	for i := 0; i < len(chunks); i++ {
		wg.Add(1)
		go func(i int) {
			result[i] = calculateWithCache(cache, pairs, chunks[i])
			wg.Done()
		}(i)
	}
	wg.Wait()
...
func calculateWithCache(cache map[string]string, pairs map[string]rune, template string) string {
	...
	result := cache[template[:CacheLen]]
	last := 0
	for i := CacheLen * 2; i+2*CacheLen < len(template); i += CacheLen {
		result = merge(pairs, result, cache[template[i-CacheLen:i]])
		last = i
	}

	return merge(pairs, result, calculate(pairs, template[last:]))
