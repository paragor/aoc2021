...
func main() {
	...
	for _, str := range strs {
		...
		//fmt.Println(numbers)
		c := 0
		c += countOne(strings.Split(strings.TrimSpace(row[1]), " "), 2)
		c += countOne(strings.Split(strings.TrimSpace(row[1]), " "), 3)
		c += countOne(strings.Split(strings.TrimSpace(row[1]), " "), 4)
		c += countOne(strings.Split(strings.TrimSpace(row[1]), " "), 7)
		fmt.Println(c)
		s += c
	}
	fmt.Println(s)
...
func dnumberToNumber(dnumber string, wired map[string]string) int {
	...
	x, ok := beta[SortString(dnumber)]
	if !ok {
		fmt.Println(wired, rwired)
		fmt.Println(alpha, beta)
		fmt.Println(SortString(dnumber))
		//panic("wtf dnumber")
	}
